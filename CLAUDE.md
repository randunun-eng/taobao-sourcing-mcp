...
>
> **`add_to_cart_batch` (many variants in ONE page visit) is UNRELIABLE — do not use it for multi-variant orders.** After each 加入购物车, a `dialogContent--…` "成功加入购物车" success dialog (z≈1e9) pops and the SKU panel re-renders; the *next* item's chip clicks then silently fail to register while Playwright still reports success → the line is **falsely confirmed but never added**. Observed: of 27 attempted packs only 13 landed (all 母对母 + most 黑色 dropped), yet every line reported ✓. **Correct method: one variant per FRESH navigation, click-and-validate each selection (per the rule above) before 加入购物车, then reconcile against the authoritative cart data XHR** (mtop cart/bag/trade response — parse line `sku.skuId` + `quantity`; ignore the null-`quantity` placeholder line objects). DOM-scraping the cart is unreliable (virtualized + the product title itself contains 母对母公对公对母, inflating text counts).
>
