...
> **✅ STATUS: v1.0 — built, audited, locked (2026-06-04).** All 7 phases are implemented and tagged (`phase-0-done … phase-6-done`); the base repo `JeremyDong22/taobao_mcp` was cloned to `_base_repo/` for recon (see `NOTES.md`). **Two forensic-audit rounds (5 detectives)** then found and FIXED **4 CRITICAL + 9 HIGH + ~25 MEDIUM/LOW** bugs — including: single-SKU pricing, `¥1,299`→`1.0`, the search price-pick (was grabbing the struck-through `优惠前` price), variant↔review cross-contamination, and the big one — **reading 参数 specs + variant-linked reviews straight from the embedded HTML** (`componentsVO`), which removed a redundant second navigation, an empty-specs gap, and a swallowed-captcha. **68 tests pass; live e2e green.** What exists: the FastMCP server (**8 tools** — login, session_status, search, fetch_product[+deep_price], fetch_reviews, **track_orders** [+取件码 digest], **add_to_cart** [gated], export_xlsx), extraction (per-SKU prices, variant-linked reviews, search), xlsx/markdown output, the sourcing Skill + supplier templates, hardening + evals. The "live co-browse" mode (below) remains a manual fallback.
>
> **Built since v1.0 (2026-06-04):** **(a) `taobao_track_orders`** — read-only daily digest: reads 已买到的宝贝, drills each active order's logistics page (dinamic frame) for status + carrier/tracking# + **取件码 pickup OTP** + station, and emits a forward-to-agent Chinese message (live-validated, paced to dodge the logistics-page throttle). **(b) `taobao_add_to_cart`** — gated, reversible cart staging: preview by default, clicks 加入购物车 only on `confirm=True`, selects the variant by exact-chip-click; **never checks out, pays, or picks an address** (the China agent does). Live-validated on the P100 (added 质保3年 tier, success toast confirmed).
>
> **Next (planned, not yet built):** confirm-then-send seller comms (read Wangwang messages + send a human-confirmed reply) — the last in-scope tool (see the scope boundary lower in this section).
>
...
3. **Reuse the warm session.** Log in once via QR; never spawn fresh anonymous contexts mid-run.
